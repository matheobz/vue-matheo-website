...
document.addEventListener("DOMContentLoaded", () => {
  ...
  function ensureOrbsStayWithinBounds() {
    ...
    orbPositions.forEach((pos, index) => {
      ...
      if (x + orbRect.width > containerRect.width)
        x = containerRect.width - orbRect.width;
      if (x < 0) x = 0;
      if (y + orbRect.height > containerRect.height)
        y = containerRect.height - orbRect.height;
      if (y < 0) y = 0;
  ...
  function animateOrb(index) {
    ...
    if (x <= 0 || x + orbRect.width >= containerRect.width)
      orbPositions[index].dx = -dx;
    if (y <= 0 || y + orbRect.height >= containerRect.height)
      orbPositions[index].dy = -dy;
...
@media (max-width: 510px) {
  .orb1 {
  @include orb-style(60vw, $orb-color1, 1);
  z-index: -2;
}

.orb2 {
  @include orb-style(70vw, $orb-color2);
}

.orb3 {
  @include orb-style(50vw, $orb-color3);
}

.orb4 {
  @include orb-style(82vw, $orb-color4, 1);
}
